...
console.log(isThisAnEmail("valid@email.com"));
...
const pet = { name: "Teo", age: 14 };
console.log(deleteProp(pet, "age"));

/* ESERCIZIO 12
  Scrivi una funzione chiamata "newestMovie" che trova il film più recente nell'array "movies" fornito.
*/
const newestMovie = () => {
  return movies.reduce((newest, current) => (current.year > newest.year ? current : newest));
};
...
const searchByTitle = (str) => {
  return str.filter(movie => movie.title.toLowerCase().includes(title.toLowerCase()));
};
...
const searchAndDivide = (movies, title) => {
  return movies.reduce((str, movie) => {
    if (movie.title.toLowerCase().includes(title.toLowerCase())) {
      str.match.push(movie);
    } else {
      str.unmatch.push(movie);
    }
    return acc;
  }, { match: [], unmatch: [] });
};
...
const printTdText = () => {
  ...
  tdElements.forEach(td => console.log(td.textContent));
};
